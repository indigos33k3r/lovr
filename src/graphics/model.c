...
Model* lovrModelCreate(ModelData* data) {
  ...
  if (data->viewCount > 0) {
    ...
    for (int i = 0; i < data->viewCount; i++) {
      ...
      model->buffers[i] = lovrBufferCreate(view->length, (uint8_t*) blob->data + view->offset, USAGE_STATIC, false);
    }
  }
  ...
  if (data->primitiveCount > 0) {
    ...
    for (int i = 0; i < data->primitiveCount; i++) {
      ...
      if (primitive->indices >= 0) {
        ...
        lovrMeshSetIndexBuffer(model->meshes[i], model->buffers[accessor->view], accessor->type == U16 ? 2 : 4);
      }
    }
  }
